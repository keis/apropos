'''Generic state tracker with persistence.'''
...
class Storage(object):
    ...
    def load(self):
        ...
        Each item and it's associated state will be generated by calling this
        method.
        '''
...
class Tracker(object):
    ...
    Track state of objects, each state may have data associated to it making
    ...
    # to not hide programming errors when catching KeyError in client code.
    class Container(dict):
        '''dict specialisation that raises UnknownState'''

        def __missing__(self, key):
            raise UnknownState(key)

    # create the dictionary for the named state. Subclasses may override to
    # provide customised behaviour.
    @classmethod
    def state(cls, name):
        return {}
    ...
    def is_alias(cls, state):
        '''Create a function that can be used as a alias for an `in state` query.
    ...
    def __init__(self, states, storage=None):
        ...
        self._states = self.Container(((s, self.state(s)) for s in states))
